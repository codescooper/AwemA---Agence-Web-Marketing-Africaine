...
1. Créer `departements/<dept>/clients/<nouveau-client>/_donnees/client.json`
   (copier celui de `exemple-client`, adapter id/nom/secteur/handles).
...
python3 scripts/connect-reseaux.py --meta departements/marketing/clients/exemple-client
python3 outils/_data/build.py
```
...
```bash
python3 scripts/connect-reseaux.py --manuel departements/marketing/clients/exemple-client export.csv
python3 outils/_data/build.py
```
